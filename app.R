      ...
      card(card_header("Step 3 — Generate ARS JSON & validate the spec"),
        card_body(
          ...
          div(class = "alert alert-warning small",
              icon("clock"), " This runs the LLM once per TLF section, so the ",
              "time scales with how many sections your shell has (roughly a few ",
              "seconds per section). Keep the tab open while it runs."),
